...
import { Loader2, Send, Settings, RefreshCw, AlertTriangle, Plus, X, Upload, ListOrdered, LogOut } from 'lucide-react';
...
export default function Admin() {
  ...
  // refresh matches (admin uses matches with no winner)
  ...
  useEffect(() => {
    if (isAuthenticated) {
      refreshMatches();
      getRankings().then(data => setRankingsCache(data || {})).catch(() => {});
    }
  }, [isAuthenticated]);
  ...
  const handleRankingUpdate = async () => {
    ...
    } finally { setIsLoading(false); setTimeout(() => setStatus(null), 2000); }
  };
  ...
  const handleTossUpdate = async () => {
    ...
    setIsLoading(true);
    try {
      await updateToss({ matchId: selectedMatchId, winner: activeMatch?.info?.teams?.[0] || '', decision: 'bat' });
  ...
  const handleScore = async (runs: number, extraType: string | null = null, isWicket: boolean = false) => {
    if (!selectedMatchId) { setStatus('Error: Select Match'); setTimeout(() => setStatus(null), 2000); return; }
    setIsLoading(true);
    let actionDesc = `${runs} Run${runs !== 1 ? 's' : ''}`;
    if (extraType) actionDesc += ` (${extraType})`;
    if (isWicket) actionDesc = "WICKET!";
    try {
      await updateScore({ matchId: selectedMatchId, runs, extraType, isWicket, comment: actionDesc });
    ...
    } finally { setIsLoading(false); setTimeout(() => setStatus(null), 1500); }
  };
  ...
  const handleAddMatchRow = () => setMatches(prev => [...prev, { info: { teams: ['', ''], dates: [''] } }]);
  ...
  const handleSubmitCreate = async () => {
    ...
    try {
      ...
      if (createType === 'series') {
        if (!eventName || teams.some(t => !t) || !seriesConfig.startDate || !seriesConfig.venue) { alert("Please fill all fields."); setIsLoading(false); return; }
        const start = parseISO(seriesConfig.startDate);
        for (let i = 0; i < seriesConfig.numMatches; i++) {
          const matchDate = addDays(start, i * 2);
          matchesToCreate.push({ format: seriesConfig.format, date: format(matchDate, 'yyyy-MM-dd'), venue: seriesConfig.venue, number: i + 1, teams: teams, players: playerDataJson?.info?.players || {} });
        }
      } else {
        if (!eventName || teams.length < 3 || teams.some(t => !t) || !tournamentConfig.startDate || !tournamentConfig.venue) { alert("Need details for tournament."); setIsLoading(false); return; }
        const start = parseISO(tournamentConfig.startDate);
        let matchCount = 0;
        for (let i = 0; i < teams.length; i++) {
          for (let j = i + 1; j < teams.length; j++) {
            const matchDate = addDays(start, matchCount);
            matchesToCreate.push({ format: tournamentConfig.format, date: format(matchDate, 'yyyy-MM-dd'), venue: tournamentConfig.venue, number: matchCount + 1, teams: [teams[i], teams[j]], players: playerDataJson?.info?.players || {} });
            matchCount++;
          }
        }
      }

      await createMatch({ type: createType, name: eventName, teams, matches: matchesToCreate });
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-4 space-y-6">
              <div className="bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-lg">
                ...
                <select className="w-full bg-gray-900 border border-gray-600 text-white p-3 rounded-lg text-sm mb-4" value={selectedMatchId} onChange={(e) => setSelectedMatchId(e.target.value)}>
                ...
                {activeMatch && hasToss && (
                  <button onClick={handleChangeInnings} className="w-full flex items-center justify-center gap-2 bg-red-900/50 hover:bg-red-900 border border-red-800 text-red-200 text-xs font-bold py-3 rounded transition"><RefreshCw size={14} /> End Innings / Switch Team</button>
                )}
              </div>
              ...
              {activeMatch && hasToss && (
                <div className="bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-lg space-y-4">
                  ...
                  <div>
                    ...
                    <select className="w-full bg-gray-900 border border-gray-600 p-2 rounded text-sm text-white">
                      <option value="">Select Striker</option>
                      {(activeMatch?.info?.players?.[battingTeamName] || ['Batter 1']).map((p: string) => <option key={p} value={p}>{p}</option>)}
                    </select>
                  </div>
                </div>
              )}
                ...
                <div className="bg-gray-800 p-8 rounded-xl border border-yellow-600/50 shadow-lg text-center">
                  ...
                  <div className="grid grid-cols-2 gap-4 max-w-md mx-auto text-left">
                    <div>
                      ...
                      <select className="w-full bg-gray-900 border border-gray-600 p-3 rounded" value={activeMatch?.info?.teams?.[0] || ''}>
                    ...
                    <div>
                      ...
                      <select className="w-full bg-gray-900 border border-gray-600 p-3 rounded">
            ...
            {/* Create modal (kept minimal to avoid verbosity) */}
            {isCreateModalOpen && (
