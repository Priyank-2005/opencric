...
export default function Admin() {
  ...
  const refreshMatches = async () => {
    try {
      ...
      const activeMatches = Array.isArray(data) ? data.filter((m: any) => !m.info.outcome?.winner) : [];
  ...
  // initial load + poll for updates when authenticated
  useEffect(() => {
    let iv: any;
    if (isAuthenticated) {
      refreshMatches();
      getRankings().then((data) => setRankingsCache(data || {})).catch(console.error);
      // poll every 15 seconds to keep admin UI fresh
      iv = setInterval(refreshMatches, 15000);
    }
  ...
  useEffect(() => {
    ...
    }).catch(e => console.error('getMatchById error', e)).finally(() => setIsLoading(false));
  }, [selectedMatchId]);
  ...
  const handleTossUpdate = async () => { if (!selectedMatchId) return; setIsLoading(true); try { await updateToss({ matchId: selectedMatchId, winner: tossWinner, decision: tossDecision }); setStatus('Toss Updated!'); const updated = await getMatchById(selectedMatchId); setActiveMatch(updated); } catch (e) { setStatus('Error updating toss'); console.error(e); } finally { setIsLoading(false); setTimeout(() => setStatus(null), 2000); } };
  const handleChangeInnings = async () => { if (!confirm("End Innings?")) return; setIsLoading(true); try { await changeInnings({ matchId: selectedMatchId }); setStatus('Innings Changed'); const updated = await getMatchById(selectedMatchId); setActiveMatch(updated); setStriker(''); setBowler(''); } catch(e) { setStatus('Error changing innings'); console.error(e); } finally { setIsLoading(false); } };
  const handleScore = async (runs: number, extraType: string | null = null, isWicket: boolean = false) => { if (!selectedMatchId || !striker || !bowler) { setStatus('Error: Select Striker & Bowler'); setTimeout(() => setStatus(null), 2000); return; } setIsLoading(true); setStatus(null); let actionDesc = `${runs} Run${runs !== 1 ? 's' : ''}`; if (extraType) actionDesc += ` (${extraType})`; if (isWicket) actionDesc = "WICKET!"; try { await updateScore({ matchId: selectedMatchId, runs, extraType, isWicket, batter: striker, bowler: bowler, comment: isWicket ? `WICKET! ${striker} is out.` : `${runs} runs to ${striker}.` }); setLastAction(`${actionDesc} (${striker})`); setStatus('Success'); const updated = await getMatchById(selectedMatchId); setActiveMatch(updated); } catch (e: any) { setStatus('Error: ' + (e.response?.data?.message || 'Update failed')); console.error(e); } finally { setIsLoading(false); setTimeout(() => setStatus(null), 1500); } };
